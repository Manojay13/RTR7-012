...
int main(void)
{
	...
	{
		ptr_iArray_Row_mk = iArray_mk[i_mk]; // 'iArray_mk[i_mk]' IS THE BASE ADDRESS OF ith ROW
		for (j_mk = 0; j_mk < NUM_COLUMNS_mk; j_mk++)
		{
			*(ptr_iArray_Row_mk + j_mk) = (i_mk + 1) * (j_mk + 1); // 'ptr_iArray_Row_mk' (That is, 'iArray_mk[i_mk]' Can Be Treated As 1D Array Using Pointers)
		}
	}
	...
	for (i_mk = 0; i_mk < NUM_ROWS_mk; i_mk++)
	{
		ptr_iArray_Row_mk = iArray_mk[i_mk];
		for (j_mk = 0; j_mk < NUM_COLUMNS_mk; j_mk++)
		{
			printf("*(ptr_iArray_Row_mk + %d) = %d \t \t At Address (ptr_iArray_Row_mk + j_mk): %p\n", j_mk, *(ptr_iArray_Row_mk + j_mk), (ptr_iArray_Row_mk + j_mk));
		}
		printf("\n\n");
	}
