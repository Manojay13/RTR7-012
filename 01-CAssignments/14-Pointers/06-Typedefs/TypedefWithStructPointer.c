...
int main(void)
{
	...
	typedef struct MyData_mk* MyDataPtr_mk;

	MyDataPtr_mk pData_mk;

	//code
	printf("\n\n");

	pData_mk = (MyDataPtr_mk)malloc(sizeof(struct MyData_mk));
	if (pData_mk == NULL)
	{
	...
	pointer_to_struct_MyData_size_mk = sizeof(MyDataPtr_mk);
	...
	if (pData_mk)
	{
		// Fixed: Replaced the document line 72-73 order bug so the buffer is freed BEFORE nullifying the address handle
		free(pData_mk);
		pData_mk = NULL;
