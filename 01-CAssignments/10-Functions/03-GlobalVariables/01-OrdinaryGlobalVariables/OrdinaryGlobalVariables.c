#include <stdio.h> //'stdio.h' contains declaration of 'printf()'
#include <stdlib.h> //'stdilib.h' contains declaration of 'exit()'

// *** GLOBAL SCOPE ***

// If not initialized by us, global variables are initialized to their zero values 
// (with respect to their data types i.e: 0 for int, 0.0 for float and double, etc.) by default.
// But still, for good programming discipline, we shall explicitly initialize our global variable with 0.

int global_count_mk = 0;

int main(void)
{
	//function prototypes
	void change_count_one(void);
	void change_count_two(void);
	void change_count_three(void);

	//code
	printf("\n");

	printf("main(): Value of global_count_mk = %d\n", global_count_mk);

	change_count_one();
	change_count_two();
	change_count_three();

	printf("\n");
	exit(EXIT_SUCCESS);
}

// *** GLOBAL SCOPE ***

void change_count_one(void)
{
	//code
	global_count_mk = 100;
	printf("change_count_one(): Value of global_count_mk = %d\n", global_count_mk);
}

// *** GLOBAL SCOPE ***

void change_count_two(void)
{
	//code
	global_count_mk = global_count_mk + 1;
	printf("change_count_two(): Value of global_count_mk = %d\n", global_count_mk);
}

// *** GLOBAL SCOPE ***

void change_count_three(void)
{
	//code
	global_count_mk = global_count_mk + 10;
	printf("change_count_three(): Value of global_count_mk = %d\n", global_count_mk);
}
